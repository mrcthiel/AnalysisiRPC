...
void eff_clust_curve(int sn, int count, ...){
	...
	vector<double> clust_mult_v;
        vector<double> clust_size_var_v;
        vector<double> clust_size_ent_v;
	clust_size_v.clear();
	clust_mult_v.clear();
        clust_size_var_v.clear();
        clust_size_ent_v.clear();

	for(int j=0; j<count; j++){
		string fileName("");
		fileName = Form("ScanId_%d/HV%d/_HV_%d_SN_%d_muons_Cluster_size.root",sn,(j+1),(j+1),sn);
		TFile _file0(fileName.c_str(),"read");
		TCanvas* n =(TCanvas*)_file0.Get("c42C");
		TPad* nn =(TPad*)n->GetPrimitive("pad42C");
		TH1F *mm = (TH1F*)nn->GetPrimitive("hClusterSize");
		clust_size_v.push_back(mm->GetMean());
                clust_size_var_v.push_back(mm->GetRMS());
                clust_size_ent_v.push_back(mm->Integral());
	}

	for(int j=0; j<count; j++){
		string fileName("");
		fileName = Form("ScanId_%d/HV%d/_HV_%d_SN_%d_muons_N_Clusters.root",sn,(j+1),(j+1),sn);
		TFile _file0(fileName.c_str(),"read");
		TCanvas* n =(TCanvas*)_file0.Get("c42C");
		TPad* nn =(TPad*)n->GetPrimitive("pad42C");
	...
	for(int i=0;i<wp.size();i++){
		...
		for(int j=0; j<count; j++){
			...
			g_clust_size->SetPoint(j,(HVs.at(j)*1000),clust_size_v.at(j));
                        g_clust_size->SetPointError(j,1,clust_size_var_v.at(j)/sqrt(clust_size_ent_v.at(j)));
