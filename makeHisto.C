...
void makeHisto(Int_t rn) {
	...
	c4->SaveAs(s);
	s.Form("plots_904/run_%d_2D_bc0timevsStrips.pdf",rn);
	c4->SaveAs(s);
	//strip vs time ps
	...
	TH2F *hist5= new TH2F("hist5", "Return strip time",60,220,280,250,0,250);
	//TH2F *hist5= new TH2F("hist5", "Return strip time",20,258,278,32,0,32);
	evt->Draw("100*(m_fpga(frame))+m_channel(frame):m_time(m_traw(frame))>>hist5");
	...
	c5->cd();
   	c5->Update();
	s.Form("plots_904/run_%d_2D_timevsChannel.root",rn);
	c5->SaveAs(s);
	s.Form("plots_904/run_%d_2D_timevsChannel.pdf",rn);
	c5->SaveAs(s);
	...
	c9->cd();
	c9->Update();
	s.Form("plots_904/run_%d_projY.root",rn);
        c9->SaveAs(s);
	s.Form("plots_904/run_%d_projY.pdf",rn);
        c9->SaveAs(s);
	...
	c6->cd();
	c6->Update();
	s.Form("plots_904/run_%d_StripsAllhits.root",rn);
	c6->SaveAs(s);
	s.Form("plots_904/run_%d_StripsAllhits.pdf",rn);
	c6->SaveAs(s);
	TCanvas *c2_ = new TCanvas("c2_","The Ntuple canvas",200,10,780,780);
        ...
        c2_->cd();
        c2_->Update();
        s.Form("plots_904/run_%d_stripsignalTime.root",rn);
        c2_->SaveAs(s);
        s.Form("plots_904/run_%d_stripsignalTime.pdf",rn);
        c2_->SaveAs(s);
