...
void eff_curve_rate(int sn, int count, ...){
	...
	std::vector<float> effnocut_on, effloose_on, effmedium_on, efftight_on, effhr_on, efflr_on, effnocut_out, effloose_out, effmedium_out, efftight_out, effhr_out, efflr_out, effcluster_on, effcluster_out, effcluster2_on, effcluster2_out, gamma_cluster_rate;
	HVs.clear();
	effnocut_on.clear();
	effloose_on.clear();
	effmedium_on.clear();
...
//		cout << "HV_low: " << HV_low << ";  gcr_low: " << gcr_low << endl;
//              cout << "HV_up: " << HV_up << ";  gcr_up: " << gcr_up << endl;
		...
		string outfile_png = Form("Efficiency_SN%d_%s.png", sn, wp.at(i));
		string outfile_pdf = Form("Efficiency_SN%d_%s.pdf", sn, wp.at(i));
		string outfile_root =Form("Efficiency_SN%d_%s.root", sn, wp.at(i));
